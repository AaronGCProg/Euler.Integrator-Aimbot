...
update_status ModuleScene::Update(float dt) {
	...
	if (App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_DOWN) {
		...
		if (body_index != -1 && App->physics->world->objects_array[body_index]->type == COLL_DYNAMIC) {
			App->physics->world->objects_array[body_index]->type = COLL_STATIC;
			mouse_joint = true;
		}
	}
	...
	// Restores previous conditions when mouse stops being pressed
		...
		App->physics->world->objects_array[body_index]->type = COLL_DYNAMIC;
		mouse_joint = false;
	}
	// -----------------------------------------------------------------------------------------------------
